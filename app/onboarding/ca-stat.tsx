...
import { useOnboarding, topCaIntent } from "@/lib/onboarding-store";
import { CA_STAT } from "@/lib/ca-content";

export default function CaStat() {
  const { state } = useOnboarding();
  const { width } = useWindowDimensions();
  const compact = width < 640;

  const intent = topCaIntent(state.caIntents) ?? "client_chase";
  const { big, line } = CA_STAT[intent];
  const bigSize = compact ? 60 : width < 880 ? 88 : 120;
        ...
        <View className="w-full self-center px-5" style={{ maxWidth: 720, paddingTop: compact ? 32 : 80 }}>
          ...
          <View className="flex-row items-center gap-2 mb-10">
            ...
            <Text className="text-sm text-slate-600 flex-1" style={{ lineHeight: 22 }}>
              That's not a Riko number. That's the Indian CA's reality we're fixing.
            </Text>
          </View>

          <View className="items-start">
            <Pressable
              onPress={() => router.push("/onboarding/ca-outcome")}
