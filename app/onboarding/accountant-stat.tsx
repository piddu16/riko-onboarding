...
import { useOnboarding, topAccountantIntent } from "@/lib/onboarding-store";
import { ACC_STAT } from "@/lib/accountant-content";
...
export default function AccountantStat() {
  ...
  const { big, line } = ACC_STAT[intent];
        ...
        <View className="w-full self-center px-5" style={{ maxWidth: 720, paddingTop: compact ? 32 : 80 }}>
          ...
          <View className="flex-row items-center gap-2 mb-10">
            ...
            <Text className="text-sm text-slate-600 flex-1" style={{ lineHeight: 22 }}>
              That's not a Riko number. That's the Indian accountant's reality we're fixing.
            </Text>
          </View>

          <View className="items-start">
            <Pressable
              onPress={() => router.push("/onboarding/accountant-outcome")}
